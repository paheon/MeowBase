...
class DTree extends ClassBase {
    ...
    public function serialize(bool $currentNode = false, string $algo = 'sha256', string $key = 'secret'): string {
    ...
    public function unserialize(string $serializedData, string $algo = 'sha256', string $key = 'secret'): ?DTree {
        ...
        if (!hash_equals($calculatedHash, $hash)) {
            return null; // Hash mismatch, data integrity compromised
        }
        $newNode = unserialize($data);
        if ($newNode) {
            $newNode->parent = null;
            return $newNode;
        }
